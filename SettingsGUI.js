...
function initializeAllSettings() {
    ...
    createSetting('PauseAtZone', 'Pause At Zone', 'If above 0, AT will pause the game as soon as it reaches this zone.', 'value', 0, null, 'Core');
    ...
    createSetting('HumaneMapSafety', 'Humane Map Safety', 'To be used with Humane Mode.<br><br>Values above above zero will cause AT to only enter maps if it call survive that many hits on average while in it.<br>Any negative value will cause AT to only enter maps it can out block.<br>Use 0 to disable this check entirely.', 'value', -1, null, 'Challenges');
    createSetting('HumaneVoidMapSafety', 'Humane Void Map Safety', 'To be used with Humane Mode, and will completely replace both Num Hits and Void Num Hits Mult.<br><br>Values above zero will cause AT to farm enough that it can survive that many hits on average before entering void maps.<br>Any negative value will cause AT to only enter void maps if it can out block them (including crits!).<br>Use 0 to disable this check entirely.', 'value', -1, null, 'Challenges');
    createSetting('HumanePauseBeforeFailing', 'Humane Pause Before Failing', 'To be used with Humane Mode.<br><br>Click this button to have  AT pause the game before your valuable trimps can be killed by the big bad guys', 'boolean', true, null, 'Challenges');
